...
import (
	...
	"github.com/hatcher/common/agent/history"
...
type TextContent struct {
	...
	Attachments []MiniFileAttachment `json:"attachments,omitempty"`
}

type MiniFileAttachment struct {
	ID   int64  `json:"id"`
...
func (dm *DataMessage) GetSimpleMessage() string {
	var simpleMessage string
	for _, part := range dm.Parts {
		if part.Type == "text" {
			convert, err := util.Convert[TextContent](part.Data)
			if err != nil {
				logs.Errorf("转换消息失败: %v", err)
				return ""
			}
			simpleMessage = convert.Text
			break
		}
	}
