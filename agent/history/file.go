...
func NewService(q *db.Queries, db *gorm.DB) Service {
	return &service{
...
func (s *service) GetByPathAndSession(ctx context.Context, path, sessionID string) (File, error) {
	dbFile, err := s.q.GetFileByPathAndSession(ctx, db.GetFileByPathAndSessionArgs{
		Path:      path,
		SessionID: sessionID,
	})
	if err != nil {
...
func (s *service) createWithVersion(ctx context.Context, sessionID, path, content string, version int64) (File, error) {
	...
	for attempt := range maxRetries {
		...
		dbFile, txErr := s.q.CreateFile(ctx, db.CreateFileArgs{
			ID:        uuid.New().String(),
			SessionID: sessionID,
			Path:      path,
			Content:   content,
