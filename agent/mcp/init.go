		...
		for name, session := range sessions.Seq2() {
			wg.Go(func() {
				if err := session.Close(); err != nil &&
					...
					logs.Warnf("Failed to shutdown MCP client, name: %s, error: %s", name, err)
				}
			})
		}
		wg.Wait()
...
func Initialize(ctx context.Context, permissions permission.Service, cfg *config.Config) {
	...
	for name, m := range cfg.MCP {
		if m.Disabled {
			...
			logs.Debugf("skipping disabled mcp, name:%s", name)
			continue
		}
		...
		go func(name string, m config.MCPConfig) {
			defer func() {
				...
				if r := recover(); r != nil {
					...
					logs.Errorf("panic in mcp client initialization, name:%s, err:%v", name, err)
				}
			}()
			...
			if err != nil {
				logs.Errorf("error listing tools, error：%v", err)
				updateState(name, StateError, err, nil, Counts{})
				session.Close()
				return
			}
			...
			if err != nil {
				logs.Errorf("error listing prompts, err:%v", err)
				updateState(name, StateError, err, nil, Counts{})
				session.Close()
				return
			}
...
func createSession(ctx context.Context, name string, m config.MCPConfig, resolver config.VariableResolver) (*mcp.ClientSession, error) {
	...
	if err != nil {
		updateState(name, StateError, err, nil, Counts{})
		logs.Errorf("error creating mcp client, name:%s, err:%v", name, err)
		cancel()
		cancelTimer.Stop()
		return nil, err
	}
	...
	client := mcp.NewClient(
		...
		&mcp.ClientOptions{
			...
			LoggingMessageHandler: func(_ context.Context, req *mcp.LoggingMessageRequest) {
				logs.Infof("MCP log, name:%s, data:%s", name, req.Params.Data)
			},
		},
	)
	...
	if err != nil {
		...
		logs.Errorf("MCP client failed to initialize，name：%s, err：%v", name, err)
		cancel()
		cancelTimer.Stop()
		return nil, err
	}
	...
	cancelTimer.Stop()
	logs.Infof("MCP client initialized，name：%s", name)
	return session, nil
}
